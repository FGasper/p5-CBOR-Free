...
static inline UV _parse_for_uint_len2( pTHX_ decode_ctx* decstate ) {
    ...
    switch (*decstate->curbyte & CONTROL_BYTE_LENGTH_MASK) {
        ...
        default:
            ret = (uint8_t) *decstate->curbyte & CONTROL_BYTE_LENGTH_MASK;
...
void _decode_hash_entry( pTHX_ decode_ctx* decstate, HV *hash ) {
    ...
    switch (CONTROL_BYTE_MAJOR_TYPE(*decstate->curbyte)) {
        case CBOR_TYPE_UINT:
        ...
        case CBOR_TYPE_UTF8:
            ...
            if (!my_key_has_sv) {
                ...
                if (SHOULD_VALIDATE_UTF8(decstate, CONTROL_BYTE_MAJOR_TYPE(*decstate->curbyte))) {
...
SV *cbf_decode_one( pTHX_ decode_ctx* decstate ) {
    ...
    switch (CONTROL_BYTE_MAJOR_TYPE(*decstate->curbyte)) {
        ...
        case CBOR_TYPE_TAG:
            ...
            U8 value_major_type = CONTROL_BYTE_MAJOR_TYPE(*decstate->curbyte);
